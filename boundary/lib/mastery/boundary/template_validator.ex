defmodule Mastery.Boundary.TemplateValidator do
  ...
  def validate_generator({name, generator}) when is_atom(name) and is_list(generator),
    do: check(generator != [], {:error, "can't be empty"})

  def validate_generator({name, generator}) when is_atom(name) and is_function(generator, 0),
    do: :ok
  ...
  def validate_raw(raw) when is_binary(raw),
    do: check(String.match?(raw, ~r{\S}), {:error, "can't be blank"})
